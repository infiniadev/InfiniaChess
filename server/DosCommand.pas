 ...
 *********************************************************************
 ...
 History :
 ---------
 28-11-2001 : version 2.1
              - Добавил развязку потоков, что бы избавиться от глюков в NT,
                и добавил корректную конвертацию OEM -> ANSI.
                                                    [Master BRAIN]
...
type
  ...
  private
    ...
    FThread: TDosThread;
    FTimer: TProcessTimer;
    FFinTimer: TTimer;  // Таймер для развязки
    FNewTimer: TTimer;  // Таймер для события строки
    FMaxTimeAfterBeginning: Integer;
    FMaxTimeAfterLastOutput: Integer;
    FPriority: Integer;
    ...
    procedure FOnFFinTimer(Sender: TObject);
    procedure FOnFNewTimer(Sender: TObject);
    //procedure CallOnNewLine;
  protected

  public
    EndOfThread: boolean;
    NewLineFromThread: boolean;
    NewLine:String;
    NewLineType:TOutputType;

    constructor Create(AOwner: TComponent); override;
    procedure Execute; //the user call this to execute the command
    procedure Stop; //the user can stop the process with this method
    function WaitFor: LongWord; // waiting for end of process
...
procedure RunExe(p_CommandLine: string; pp_SL: TStrings);
function _RunScriptFile(
  p_User,p_Password,p_Server: string;
  p_Directory,p_FileName: string): string;

implementation

type TCharBuffer = array[0..MaxInt - 1] of Char;
{
var   EndOfThread:boolean=False;
      NewLineFromThread:boolean=False;
      NewLine:String;
      NewLineType:TOutputType;
}
//------------------------------------------------------------------------------
...
//    Exit;
 ...
 validates the wShowWindow member.}
  ...
  pDirectory := PChar(GetWorkingDirectory);

 //spawn the child process
  if not (CreateProcess(nil, app_spawn, nil, nil, TRUE,
    CREATE_NEW_CONSOLE or FPriority, nil, pDirectory, si, pi)) then
  begin
...
//    Exit;
      ...
      if (bread <> 0) then begin
            ...
            #10:
              begin
                ...
                if Assigned(FOnNewLine) then
                  begin
                  TDosCommand(FOwner).NewLine:=Str;
                  TDosCommand(FOwner).NewLineType:=otEntireLine;
                  TDosCommand(FOwner).NewLineFromThread:=True;
                  Sleep(2);
                  end;
                Str := '';
              end;
        ...
        begin
          ...
          if Assigned(FOnNewLine) then
            begin
            TDosCommand(FOwner).NewLine:=Str;
            TDosCommand(FOwner).NewLineType:=otBeginningOfLine;
            TDosCommand(FOwner).NewLineFromThread:=True;
            Sleep(2);
            end;
          LineBeginned := True;
      ...
      //send lines in input (if exist)
        ...
        begin
          ...
          begin
            ...
            if Assigned(FOnNewLine) then
              begin
              TDosCommand(FOwner).NewLine:=Last;
              TDosCommand(FOwner).NewLineType:=otEntireLine;
              TDosCommand(FOwner).NewLineFromThread:=True;
              Sleep(2);
              end;
  ...
  if Assigned(FOnTerminated) then
   TDosCommand(FOwner).EndOfThread:=True;
end;
//------------------------------------------------------------------------------
function TDosThread.GetWorkingDirectory: string;
var
  i: integer;
begin
  if pos('\', FCommandLine) = 0 then result := GetCurrentDir
  else begin
    i := length(FCommandLine);
    while FCommandLine[i] <> '\' do dec(i);
    result := copy(FCommandLine, 1, i);
  end;
end;
//------------------------------------------------------------------------------
procedure TDosCommand.FOnFFinTimer(Sender: TObject);
begin
  if not EndOfThread then exit;
  EndOfThread:=False;
  FOnTerminated(FOwner); // Вызываем событие конца
end;

procedure TDosCommand.FOnFNewTimer(Sender: TObject);

  function OEM2Ansi(S:String):String;
  var Ansi_CODE,OEM_CODE:String;
      i:integer;
  begin
    OEM_CODE:= 'ЂЃ‚ѓ„…†‡€‰Љ‹ЊЌЋЏђ‘’“”•–—™љ›њќћџ ЎўЈ¤Ґ¦§Ё©Є«¬­®Їабвгдежзийклмнопьс'+#255;
    Ansi_CODE:='АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдежзийклмнопрстуфхцчшщъыьэюя№ё'+#32;
    Result:=S;
    for i:=1 to Length(Result) do
     if Pos(Result[i],OEM_CODE)<>0 then Result[i]:=Ansi_CODE[Pos(Result[i],OEM_CODE)];
  end;

begin
  if not NewLineFromThread then exit;
  NewLineFromThread:=False;
  //Synchronize(CallOnNewLine);
  FOnNewLine(FOwner,Oem2Ansi(NewLine),NewLineType); // Вызываем событие строки
end;
...
begin
  ...
  if not (csDesigning in ComponentState) then
   begin
   FFinTimer:=TTimer.Create(nil);
   FFinTimer.OnTimer:=FOnFFinTimer;
   FFinTimer.Interval:=5;
   FFinTimer.Enabled:=False;
   //-----  Пару таймеров следить за потоком
   FNewTimer:=TTimer.Create(nil);
   FNewTimer.OnTimer:=FOnFNewTimer;
   FNewTimer.Interval:=1;
...
begin
  ...
  begin
    ...
    FFinTimer.Enabled:=True;
    FNewTimer.Enabled:=True;
  end;
end;
...
function TDosCommand.WaitFor: LongWord;
begin
...
procedure Register;
begin
  RegisterComponents('Master Components', [TDosCommand]);
end;

//------------------------------------------------------------------------------
//==============================================================================
//==========    Добавил Urise 22.03.2006     ===================================
//==============================================================================
procedure RunExe(p_CommandLine: string; pp_SL: TStrings);
var
  DS: TDosCommand;
  CurDir: string;
begin
  if pos(':',p_CommandLine)>0 then begin
    CurDir:=GetCurrentDir;
    SetCurrentDir(ExtractFileDir(p_CommandLine));
  end;
  DS := TdosCommand.Create(nil);
  DS.OutputLines := pp_SL;
  DS.CommandLine:=p_CommandLine;
  //showmessage(ExtractFileDir(p_CommandLine)+#13#10+GetCurrentDir+#13#10+DS.CommandLine);
  DS.Execute;
  DS.WaitFor;
  DS.Free;
  if pos(':',p_CommandLine)>0 then
...
begin
  CurDir:=GetCurrentDir;
  if p_Directory<>'' then SetCurrentDir(p_Directory);
  s:='sqlplus.exe '+p_User+'/'+p_Password;
  if p_Server<>'' then s:=s+'@'+p_Server;
  s:=s+' @'+p_FileName;
  try
    RunExe(s,nil);
  finally
